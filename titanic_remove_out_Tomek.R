...
setwd('C:\\Users\\LG\\Desktop\\Kaggle-master')
...
# 아웃라이어 제거 
input = c('Age', 'SibSp', 'Parch', 'Fare', 'family_size')
Tomek_train<-ubTomek(X=titanic.train[,input], Y= titanic.train[,'Survived'])
Tomek_train$id.rm
...
fit_random_forest<- randomForest(formula = Survived ~ title + Pclass + Sex + Age 
                                 ...
                                 , data = titanic.train[-Tomek_train$id.rm,])


pred<- predict(fit_random_forest, titanic.test)
submission$Survived<- pred

write.csv(submission, '20190114.csv', row.names = FALSE)
